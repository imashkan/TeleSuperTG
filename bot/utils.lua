...
function is_channel_msg( msg_channel )
    if msg.to.type == 'channel' then
    return true
  end
  return false
...
function Kick_by_reply(extra, success, result)
  if result.to.type == 'channel' then
    local chat = 'channel#id'..result.to.id
    if tonumber(result.from.id) == tonumber(our_id) then -- Ignore bot
      return "I won't kick myself"
    end
    if is_momod2(result.from.id, result.to.id) then -- Ignore mods,owner,admin
    ...
    channel_kick_user(chat, 'user#id'..result.from.id, ok_cb, false)
  else
    return 'Use This in Your Groups'
  end
end

-- Kick by reply for channel
function Kick_by_reply_channel(extra, success, result)
  if result.to.type == 'channel' then
